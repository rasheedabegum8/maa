    ...
    <!-- Right Panel -->
        ...
        <div class="content">
                                ...
                                <div class="col-4">
                                   <c:if test='${roleid==1||roleid==2}'>
                                    <div class="addButton" style="float: right">
                                        <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#associtionAddModal"><i class="fa fa-plus"></i></button>
                                    </div>
                                    </c:if>
                                </div>
                            </div>
                                ...
                                <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                                    <div class="card-header user-header headCustm alt">
										<div class="iconsDiv" style="float: right">
											...
											'${aso.startdate}','${aso.enddate}','${aso.contact}','${aso.email}','${aso.pic}')"><i class="fa fa-edit"></i></a>
											</c:if>
										</div>
                                        <div class="media">
                                        <c:choose>
                                        ...
                                        <c:otherwise>
                                        <a href="#">
                                            <img class="align-self-center rounded-circle mr-1 mt-3" style="width:90px; height:90px;" alt="" src="resources/maa-images/u1.png">
                                        </a>
                                        </c:otherwise>
                                        </c:choose>
                                            ...
                                            <div class="media-body pt-4">
                                                ...
                                                <p><i class="fa fa-user"></i>&nbsp;${aso.memType} <br>
                                                    ...
                                                    <i class="fa fa-calendar"></i>&nbsp;${aso.startdate} To ${aso.enddate}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
          ...
          <span aria-hidden="true">&times;</span>
                            ...
                            <form action="SaveAssoiciationUser" method="POST">
                            <div class="modal-body">
                              <div class="row">
                                    <div class="col-md-6">
                                    ...
                                    <input type="hidden" id="prtyId" name="prtyId" placeholder="Ex:PTY1" value="${prptyid}" class="form-control">
                                     ...
                                     <input type="radio" id="chkBox" name="chkBox" value="O" onclick="getOwner('${prptyid}')"> Owner
                                      <input type="radio" id="chkBox" name="chkBox" value="T" onclick="getTenant('${prptyid}')"> Tenant
                                     </div>
                                   ...
                                   <div class="form-group">
                                            ...
                                            <input type="text" id="flatno" name="flatno" class="form-control" readonly>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="form-group">
                                            <label for="company" class=" form-control-label">Member Type</label>
                                            <div id="custm-select">
                                                <select class="custom-select" id="memType" name="memType">
										...
										<div class="row">
											<div class="col">
												...
												<div class="form-group">
                                            <label for="company" class=" form-control-label">Start Date:</label>
                                            <input type="date" id="strtDate" name="strtDate" placeholder="Enter id number" class="form-control">
                                        </div>
											</div>
											<div class="col">
												<div class="form-group">
                                            <label for="company" class=" form-control-label">To Date:</label>
                                            <input type="date" id="endDate" name="endDate" placeholder="Enter id number" class="form-control" >
                                        </div>
                                        <div class="form-group">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="company" class=" form-control-label">Member Type</label>
                                            <div id="custm-select">
                                                <select class="custom-select" id="Ed_memType" name="Ed_memType">
                                                        <option selected>Select Member</option>
                                                        <option value="President">President</option>
                                                        <option value="Vice President">Vice President</option>
                                                        <option value="Secretary">Secretary</option>
										...
										<div class="row">
											<div class="col">
												<div class="form-group">
                                            <label for="company" class=" form-control-label">Start Date:</label>
                                            <input type="date" id="Ed_strtDate" name="Ed_strtDate" placeholder="Enter id number" class="form-control">
                                        </div>
											</div>
											<div class="col">
												<div class="form-group">
                                            <label for="company" class=" form-control-label">To Date:</label>
                                            <input type="date" id="Ed_endDate" name="Ed_endDate" placeholder="Enter id number" class="form-control" >
                                        </div>
                                          <div class="form-group">
	    ...
	    success: function(response){ 
	              ...
	              if(response!=null){
		              for(var i=0;i<response.length;i++){
			              ...
			              var ownerVal=response[i].ownerid+'_'+response[i].email+'_'+ownername+'_'+response[i].pic+'_'+response[i].flatno+'_'+response[i].contact;
	            	  $('#usergroup').append(
								'<option value=' +ownerVal+ '>' + ownername
										+ '</option>');
		              }
	            }
...
function getTenant(prtyid){
	...
	$.ajax({
	    ...
	    success: function(response){  
	              ...
	              if(response!=null){
		              for(var i=0;i<response.length;i++){
			              alert(response[i].flatno);
		            	  var tntname=response[i].tenantname;
				            tntname=tntname.replace(/ /g,'-');
			              var ownerVal=response[i].tenantid+'_'+response[i].email+'_'+tntname+"_"+response[i].pics+'_'+response[i].flatno+'_'+response[i].contact;
			              
	            	  $('#usergroup').append(
								'<option value=' +ownerVal+ '>' + tntname
										+ '</option>');
		              }
	            }
...
function EditAssociation(id,name,memType,ownerid,tntid,startdate,enddate,contact,email,pic){

	var prtyid=$('#Ed_prtyId').val();
	$('#Ed_usrName').val(name);
	$('#Ed_associationId').val(id);
	//$('#Ed_memtype').val(memType);
	...
	if(ownerid!=0){
		    ...
		    success: function(response){ 
		              ...
		              if(response!=null){
			              for(var i=0;i<response.length;i++){
				              ...
				              var ownerVal=response[i].ownerid+'_'+response[i].email+'_'+response[i].ownername+'_'+response[i].pic+'_'+response[i].contact+'_'+response[i].flatno;
		            	  $('#Ed_usergroup').append(
									'<option value=' +ownerVal+ '>' + response[i].ownername
											+ '</option>');
			              }
		            }
		...
		var idTypeVal = $("select[name='Ed_usergroup']");
		 idTypeVal.find('option[text="' + name + '"]').attr('selected', true);
		}
	if(tntid!=0){
		    ...
		    success: function(response){  
		              ...
		              if(response!=null){
			              for(var i=0;i<response.length;i++){
				              ...
				              var ownerVal=response[i].tenantid+'_'+response[i].email+'_'+response[i].tenantname+"_"+response[i].pics+'_'+response[i].contact+'_'+response[i].flatno;
				              
		            	  $('#Ed_usergroup').append(
									'<option value=' +ownerVal+ '>' + response[i].tenantname
											+ '</option>');
			              }
		            }
		...
		var memtypeVal = $("select[name='Ed_memType']");
		 memtypeVal.find('option[value="' + memType + '"]').attr('selected', true);
